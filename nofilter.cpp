...
const gchar* p_src =
  "v4l2src v4l2=0  do-timestamp=1 is-live=1 ! "
...
const gchar* p_sink =
  ...
  */
  ...
  //" vp8enc error-resilient=1 bitrate=500 speed-present=superfast ! queue ! rtpvp8pay !"
  " vp8enc error-resilient=1 speed-present=ultrafast max-latency=2 quality=2 speed=7 cpu-used=10 end-usage=cbr target-bitrate=1024000 token-partitions=3 static-threshold=1000 min-quantizer=0 max-quantizer=63 threads=2 deadline=50 keyframe-max-distance=20 ! queue ! rtpvp8pay pt=96 !"
  " identity name=point3 ! "
  " queue ! "
  " udpsink host=127.0.0.1 port=5004 auto-multicast=true "
  " autoaudiosrc buffer-time=36000 ! audioconvert ! audioresample ! audio/x-raw,rate=16000,channels=1,format=S16LE ! "
...
myfile.open ("nofilter.txt");
myfile << "Begin User Without filter.\n"<<endl;
